...
import {
  ...
  COURSE_ROLES_ENUM,
} from '../model/course.enum';
import { IonicDialogService } from '../../../core/services/ionic-dialog.service';
import {
  CourseCreate,
  CourseInviteMemberConfirm,
} from '../model/course-module.interface';
...
export class CourseFeatureFacadeService {
  ...
  async inviteMemberIntoCourse(userMain: StUserMain): Promise<void> {
    ...
    const selectedCourseRole: COURSE_ROLES_ENUM =
      coursePromise.data?.selectedCourseRole;

    if (coursePublic) {
      const message = `Invite ${userMain.displayName} into course ${coursePublic.longName}`;
      const invitation = await this.inviteMemberIntoCourseConfirm(
        message,
        coursePublic,
        selectedCourseRole,
        false
  ...
  async inviteMemberIntoCourseConfirm(
    message: string,
    coursePublic: CoursePublic,
    selectedCourseRole: COURSE_ROLES_ENUM,
    disabled: boolean
  ): Promise<CourseInviteMemberConfirm> {
  ...
  async addNewCourse(courseCreate: CourseCreate) {
    ...
    const markerInvitation = createCourseInvitation(
      courseCreate.coursePublic,
    ...
    courseCreate.coursePrivate.invitedStudents.forEach((m) =>
      this.accountFeatureDatabaseService.addOrRemoveCourseInvitationForPerson(
        m,
        studentInvitation,
        true
      )
    ...
    IonicDialogService.presentToast(
      `Course ${courseCreate.coursePublic.longName} has been created`
    );
  }
}
