...
export class CourseTestFeatureDatabaseService {
  ...
  getStudentCourseTest(courseId: string, testId: string, userId: string): Observable<CourseTestTaken> {
    return this.firestore.collection(this.COURSE).doc(courseId).collection<CourseTest>(this.TESTS)
      .doc(testId).collection<CourseTestTaken>(this.TEST_TAKEN).doc(userId).valueChanges();
  }

  saveStudentCourseTest(courseTestTaken: CourseTestTaken) {
    this.firestore.collection(this.COURSE).doc(courseTestTaken.course.courseId).collection<CourseTest>(this.TESTS)
      .doc(courseTestTaken.testId).collection<CourseTestTaken>(this.TEST_TAKEN).doc(courseTestTaken.student.uid)
