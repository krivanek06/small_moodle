...
import {CourseFeatureDatabaseService, CourseFeatureStoreService} from '@app/features/course-feature';
...
export class CourseTestFeatureFacadeService {
    ...
    getAllStudentsResultsForCourseTests(
        ...
        return this.courseFeatureStoreService
            ...
            .pipe(
                switchMap((course) =>
                    this.courseTestDatabaseService.getAllStudentsResultsForCourseTests(
                        course.courseId,
                        testId
                    )
                )
    ...
    async approveCourseTest(approve: boolean, courseTest: CourseTest) {
        ...
        if (await this.presentDialog(action, courseTest)) {
            courseTest.testState = approve
                ? CourseTestStateEnum.APPROVED
                : CourseTestStateEnum.IN_PROGRESS;
            if (approve) {
                const courseTestPublic = convertCourseTestIntoCourseTestPublic(
                    courseTest
                );
                this.courseFeatureDatabaseService.addCourseTestIntoPublic(
                    courseTestPublic
                );
            }
    ...
    async startCourseTest(courseTest: CourseTestPublic): Promise<boolean> {
        ...
        const studentTest = await this.courseTestDatabaseService
            .getStudentCourseTest(courseId, testId, user.uid)
            .pipe(first())
            .toPromise();
        ...
        if (studentTest) {
            if (await this.presentDialog('continues', courseTest)) {
                ...
                IonicDialogService.presentToast(
                    `You are continuing ${studentTest.testName}`
                );
                return true;
            }
            return false;
        ...
        if (await this.presentDialog('start', courseTest)) {
            // load course test
            const test = await this.courseTestDatabaseService
    ...
    async assignMarkerOnCourseTest(takenTest: CourseTestTaken) {
        ...
        IonicDialogService.presentToast(
            `Marker ${user.displayName} has been assign on test ${courseTestTaken.testName}`
        );
    }
    ...
    async gradeCourseTest(oldTest: CourseTestTaken, modifiedTest: CourseTest) {
        ...
        const courseTest: CourseTestTaken = {
            ...
            receivedPoints: modifiedTest.questions
    ...
    private async presentDialog(
        action: string,
        {testName, course}: CourseTestPublic
    ) {
    ...
    private presentToaster(
        action: string,
        {testName, course}: CourseTestPublic
    ) {
        IonicDialogService.presentToast(
            `${testName} has been ${action} for course ${course.longName}`
        );
    }
}
