...
export class DashboardAuthenticatedFacadeService {
  ...
  async showCourseInvitation(invitation: CourseInvitation) {
    ...
    const result = await this.courseFeatureFacadeService.inviteMemberIntoCourseConfirm(
      message,
      invitation.course,
      invitation.invitedAs,
      true
    );

    if (result?.confirm) {
      await this.accountService.addOrRemoveCourseInvitationForPerson(
        this.authService.user,
        invitation,
        false
      );
      await this.accountService.saveCourseForUser(
        this.authService.user,
        invitation.course,
        invitation.invitedAs
      );
      await this.courseFeatureDatabaseService.removePersonInvitationFromCourse(
        invitation.course,
        this.authService.user,
        invitation.invitedAs
      );
      await this.courseFeatureDatabaseService.addPersonIntoCourse(
        invitation.course,
        this.authService.user,
        invitation.invitedAs
      );
      IonicDialogService.presentToast(
        `Course ${longName} invitation has been accepted`
      );
      ...
      IonicDialogService.presentToast(
        `Course ${longName} invitation has been declined`
      );
    }
  }
  ...
  async createCourse() {
    ...
    if (courseCreate) {
      ...
      IonicDialogService.presentToast(
        `Course ${courseCreate.coursePublic.longName} been created`
      );
    }
  }
}
